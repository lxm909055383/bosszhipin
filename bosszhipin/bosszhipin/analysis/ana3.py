...
#将字符串存入文件中
fr = codecs.open('word_set.txt', 'a', encoding='utf8')
for item in mydata_2:
    fr.write(item.key_word)
    fr.write(',')
fr.close()
#从文件中提取字符串并进行分词
text = codecs.open('word_set.txt',"r", encoding='utf8').read()
...
wc.generate(result)
#保存图片
wc.to_file("E:/bosszhipin/bosszhipin/bosszhipin/analysis/wordcloud.png")
#显示图片
